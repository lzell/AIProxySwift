...
open class OpenAIRealtimeSession {
    ...
    private func didReceiveWebSocketMessage(_ message: URLSessionWebSocketTask.Message) {
        ...
        case .data(let data):
            self.didReceiveWebSocketData(data)
        @unknown default:
            aiproxyLogger.error("Received an unknown websocket message format")
            self.disconnect()
        }
    }
    ...
    private func didReceiveWebSocketData(_ data: Data) {
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            aiproxyLogger.error("Received websocket data that we don't understand")
            self.disconnect()
            return
        }

        aiproxyLogger.debug("Received over ws: \(messageType)")
