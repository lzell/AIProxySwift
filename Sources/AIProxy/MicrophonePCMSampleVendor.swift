...
open class MicrophonePCMSampleVendor {
    ...
    private func convertPCM16BufferToExpectedSampleRate(_ pcm16Buffer: AVAudioPCMBuffer) -> AVAudioPCMBuffer? {
        ...
        guard let audioFormat = AVAudioFormat(
            ...
            aiproxyLogger.error("Could not create target audio format")
            return nil
        }
        ...
        guard let converter = self.audioConverter else {
            aiproxyLogger.error("There is no audio converter to use for PCM16 resampling")
            return nil
        }
        ...
        guard let outputBuffer = AVAudioPCMBuffer(
            ...
            aiproxyLogger.error("Could not create output buffer for PCM16 resampling")
            return nil
        }
        ...
        if let error = error {
            aiproxyLogger.error("Error converting to expected sample rate: \(error.localizedDescription)")
            return nil
        }
