...
//        return try ReplicateSynchronousAPIOutput<U>.deserialize(from: data)
    ...
    public func createModel(
        ...
        let responseModel: ReplicateModelResponseBody = try await self.makeRequestAndDeserializeResponse(request)
    ...
    private func actorGetPredictionResult<U: Decodable>(
        ...
        guard let host = url.host else {
            throw AIProxyError.assertion("No polling domain available for replicate poll request")
        }
        let request = try AIProxyURLRequest.createDirect(
            baseURL: host, // I think this is wrong
            path: url.path,
            body: nil,
            verb: .get,
            additionalHeaders: [
